...
export const configSchema = z.object({
    ...
    logging: z.object({
        ...
        maxSize: z.string().regex(/^\d+[kmg]b$/i).optional()
    }),
    ...
    message: "session.defaultProvider must reference a provider defined in the providers map",
...
export const getProviderSchema = (type: string) => {
    return providersSchemasMap[type as keyof typeof providersSchemasMap] ?? 
        z.object({
            model: z.string().min(1)
        }).passthrough();
};
